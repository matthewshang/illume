...
static Scene* init_scene()
{
	...
	scenebuilder_add_mesh_instance(builder,
		mesh_instance_new(0, material_emissive(vector3_create(3, 2.5, 2)),
			transform_create(
				vector3_create(-0.0f, 3.5f, 5.0f), vector3_create(2.5, 0.02, 2.5),
					matrix4_create())));
	...
	scenebuilder_add_mesh_instance(builder,
		mesh_instance_new(2, white,
			transform_create(
				vector3_create(0, 3.5, 5), vector3_create(5, 5, 5),
					matrix4_from_axis_angle(vector3_create(1, 0, 0), ILLUME_PI / 2))));
	...
	scenebuilder_add_sphere(builder, sphere_new(0.75, vector3_create(1, -0.25, 4), glass));
	scenebuilder_add_sphere(builder, sphere_new(0.75, vector3_create(-1, -0.25, 5), mirror));
	...
	}
	{
		...
		}
		{
			...
			//Camera camera = camera_create(vector3_create(0, 0, 0), 70, 4.5, 0.085);
			 Camera camera = camera_create(vector3_create(0, 1.5f, 0), 90, 1, 0);
